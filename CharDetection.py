...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("classified.txt", np.float32)                  # read in training classifications
    except:                                                                                 
        print("error, unable to open classified.txt, exiting program\n") 
        os.system("pause")
        return False                                                                       
    try:
        npaFlattenedImages = np.loadtxt("fimages.txt", np.float32)              
    except:
        print("error, unable to open fimages.txt, exiting program\n")  
        os.system("pause")
        return False                                                                    
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          
        possiblePlate.imgGrayscale, possiblePlate.imgThresh = ImageProcessing.preprocess(possiblePlate.imgPlate)
        ...
        if MainProgram.showSteps == True: 
            ...
            cv2.drawContours(imgContours, contours, -1, MainProgram.SLR_WHITE)
...
def findPossibleCharsInPlate(imgGrayscale, imgThresh):
    ...
    for contour in contours:                        # for each contour
        possibleChar = PossChar.PossibleChar(contour)
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    for currentChar in listOfMatchingChars:                                        
        ...
        cv2.rectangle(imgThreshColor, pt1, pt2, MainProgram.SLR_GREEN, 2)           
